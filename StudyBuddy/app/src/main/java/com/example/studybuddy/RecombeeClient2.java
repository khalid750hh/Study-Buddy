 ...
 *
 * IMPORTANT NOTE TO GRADER: This class comes from the Recombee Java API. It is their code.
 *  The reason why we have it copied here as RecombeeClient2.java instead of simply
 *  importing their RecombeeClient.java file directly is because we needed to edit something
 *  in their file for it to work, and the original RecombeeClient.java file is read only. That
 *  is why we have copied their file here to RecombeeClient2.java and stored it in our project's
 *  java files directory. We have only made one edit, which can be seen in the signURL([byte])
 *  method of this class starting on line 902 down below.
 */
public class RecombeeClient2 {
    ...
    public RecombeeClient2(String databaseId, String token) {
    ...
    public RecombeeClient2 setDefaultProtocol(NetworkApplicationProtocol defaultProtocol) {
        this.defaultProtocol = defaultProtocol;
        return this;
    }
    public RecombeeClient2 setBaseUri(String baseUri)
    {
    ...
    private String signUrl(String url) {
        ...
        try {
            ...
            // THIS IS THE ONLY LINE OF THEIR CODE WE CHANGED. THE ORIGINAL LINE CALLED THE STATIC
            //  METHOD Hex.encodeHex([byte]) IN ORDER TO CONVERT rawHmac FROM A BYTE TO A HEX STRING
            //  BUT THIS CREATED A CONFLICT IN THE OVERALL PROGRAM WITH ANOTHER Hex CLASS THAT DOES
            //  NOT HAVE THIS encodeHex() METHOD IN IT, THUS CRASHING THE PROGRAM WHEN IT TRIED TO
            //  FIND encodeHex() IN THE WRONG Hex CLASS WHERE IT DOESN'T EXIST. WE HAVE CHANGED THIS
            //  LINE TO USE A CUSTOM BYTE TO HEX METHOD INSTEAD, WHICH IS THE bytesToHex([byte])
            //  METHOD BELOW THIS ONE
            String sign = bytesToHex(rawHmac);
    ...
    // FROM THE TOP ANSWER ON THE FOLLOWING PAGE:
